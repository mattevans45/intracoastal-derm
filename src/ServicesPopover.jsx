...
export default function ServicesPopover() {
  ...
  return (
    ...
    >
      <button
        className="group inline-flex gap-x-2 rounded-lg px-2 py-1.5 uppercase text-[#4d4d4d] transition-all duration-500 ease-in-out focus-within:outline-none hover:rounded-lg hover:border-gray-200 hover:bg-[#30648B]/90 hover:px-2 hover:py-1.5 hover:text-white focus:outline-none focus-visible:ring focus-visible:ring-gray-500/50"
        onClick={() => setIsOpen(!isOpen)}
        aria-expanded={isOpen}
        aria-haspopup="true"
      >
        <span className="font-display shadow-gray-400/20 drop-shadow-md">
          Services
        </span>
        <ChevronDownIcon
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit="exit"
            variants={menuVariants}
            className="absolute left-0 top-7 z-50 flex flex-col gap-y-1.5 rounded-2xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5"
          >
            <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
              <div className="p-4">
                {services.map((item) => (
                  <motion.div
                    key={item.name}
                    className="group relative flex items-center justify-start gap-x-6 rounded-lg p-4 transition-all duration-500 ease-in-out focus-within:outline-none hover:rounded-lg hover:border-gray-200 hover:bg-[hsla(206,49%,37%,.80)] hover:text-white focus:outline-none focus-visible:ring focus-visible:ring-gray-500/50"
                    initial="hidden"
                    animate="visible"
                    variants={menuVariants}
                  >
                    <div className="size-9 rounded-md bg-gray-50 group-hover:bg-white">
                      <item.icon
                        className="inline-flex size-9 text-[#4d4d4d] group-hover:text-gray-500"
                        aria-hidden="true"
                      />
                    </div>
                    <div onClick={() => setIsOpen(!isOpen)}>
                      <Link
                        to={item.to}
                        className="prose font-semibold text-gray-800 group-hover:text-white"
                      >
                        {item.name}
